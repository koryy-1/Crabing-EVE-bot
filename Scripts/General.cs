...
namespace EVE_Bot.Scripts
{
    internal class General
    {
        ...
        static public void RepairShip()
        {
            ...
            var (XRepairShop, YRepairShop) = Finders.FindLocWnd("RepairShopWindow");
            ...
            for (int i = 0; i < ItemEntries.children.Length; i++)
            {
                ...
                var (XHybridWindow, YHybridWindow) = Finders.FindLocWnd("HybridWindow");
        ...
        static public bool Orbiting(string ObjectNameInGrid)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                var (X, Y) = Finders.FindObjectByWordInOverview(ObjectNameInGrid);
                if (X == 0)
                {
        ...
        static public bool OrbitViaContextMenu(string ObjectNameInGrid, string OrbitRadius)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                (X, Y) = Finders.FindObjectByWordInOverview(ObjectNameInGrid);
                if (X == 0)
                {
                    Console.WriteLine($"No {ObjectNameInGrid} in grid");
                    return false;
        ...
        static public bool ClickInContextMenu(string ContextMenuField)
        {
            ...
            var YOffset = Convert.ToInt32(ContextMenuEntry.dictEntriesOfInterest["_displayY"]);
        ...
        static public void GotoInActiveItem(string ItemInSpace, string Maneuver)
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                var (X, Y) = Finders.FindObjectByWordInOverview(ItemInSpace);
                if (X == 0)
                {
        ...
        static public void ChangeTab(string TabName)
        {
            ...
            // change tab
            var (XOverViewLoc, YOverViewLoc) = Finders.FindLocWnd("OverView");
            int XTabLoc = 0;
